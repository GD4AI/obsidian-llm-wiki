...
export const RU_TEXTS = {
    ...
    wikiIngestedMarkerFailed: 'Не удалось записать маркер wiki-ingested для «{filename}». Приём выполнен успешно, но повторный запуск не будет пропущен — используйте принудительный повторный приём или добавьте маркер вручную.',
    wikiIngestedMarkerRepaired: '[LLM Wiki] Исправление при запуске: маркер wiki-ingested повторно записан для {count} принятой(ых) заметки/заметок.',
